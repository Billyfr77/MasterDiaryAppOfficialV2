...
// Initialize Gemini
// Use a safe fallback if key is missing to prevent crash on startup, but requests will fail gracefully.
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || 'API_KEY_MISSING');
const getModel = () => genAI.getGenerativeModel({ model: 'gemini-pro-latest' });
...
module.exports = { chatWithAI, generateContent, summarizeText, analyzeQuote, analyzeBusinessHealth, getCloudAssistInsight };