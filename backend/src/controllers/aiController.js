...
const googleAdvanced = require('../services/googleAdvancedService');
...
const getInstitutionalContext = async (userId) => {
    ...
    const cortexBias = packet.cortex?.biasDirection !== 'ACCURATE' 
        ? `⚠️ CRITICAL LEARNING: You have a historical bias of ${packet.cortex?.estimationBias} (${packet.cortex?.biasDirection}). Adjust all cost estimates by this factor to correct it.` 
        : "✅ Historical estimation is accurate.";
        
    const cortexRules = packet.cortex?.associationRules?.map(r => `[Pattern] If user needs '${r.itemA}', also suggest '${r.itemB}' (Confidence: ${r.confidence})`).join('\n    ') || "No strong item patterns yet.";

    const cortexSchedule = `
    **RESOURCE TEMPORAL PRESSURE (NEXT 14 DAYS):**
    - 🔴 **CRITICAL LOAD (Do Not Book):** ${packet.cortex?.temporalPressure?.criticalStaff?.join(', ') || "None"}
    - 🟢 **OPEN CAPACITY:** ${packet.cortex?.temporalPressure?.availableCrew?.join(', ') || "Full Crew"}
    - **Fleet Activation:** ${(parseFloat(packet.cortex?.temporalPressure?.loadIndex || 0) * 100).toFixed(0)}% of workforce is active.
    `;

    const cortexRisks = `
    **RISK & SAFETY PROTOCOLS (PREDICTIVE):**
    - ⚠️ **FATIGUE ALERT:** ${packet.cortex?.fatigueRisk?.length > 0 ? packet.cortex.fatigueRisk.join(', ') + " (Recommend Rest Day)" : "No fatigue risks detected."}
...
const generateQuote = async (req, res) => {
    try {
        ...
        const systemPrompt = `
            You are "NEE V7 - SENIOR ESTIMATOR ARCHITECT". ${OMNISCIENCE_MANDATE} ${memory} 
            **Mission:** Architect a mathematically precise and VISUALLY BEAUTIFUL quote blueprint.
            
            **VISUAL ARCHITECTURE MANDATE (THE "CIRCUIT BOARD" STRATEGY):**
            You must arrange nodes on the canvas to look like a clean, organized electrical circuit. 
            DO NOT dump nodes on top of each other.
            
            1. **The Spine (Zones/Areas):** Place 'areaNode' items in a vertical column at X=0, spaced 400px apart vertically (Y=0, Y=400, Y=800...).
            2. **The Satellites (Items):** Place connected Materials/Labor to the RIGHT of their parent zone (X=400 to X=800).
               - Stagger their Y-coordinates so they fan out beautifully.
            3. **The Intelligence Layer (Power Nodes):**
               - **Estimation Prism:** Place ONE at (800, -200) to oversee the project.
               - **Material Yield:** Place near complex materials at (600, 200).
               - **Labour Estimator:** Place near staff groups at (600, -200).
            
            **PRICING ACCURACY & SANITY PROTOCOL:**
            - **'cost' / 'rate' is PER UNIT.** (e.g., $60/hour, NOT $60,000 total).
            - **'quantity' is the COUNT.** (e.g., 8 hours, NOT 8000 hours).
            - **REALISM CHECK:** For a standard house/room, totals should be $5k - $500k. If you generate >$1M, YOU ARE WRONG. Recalibrate quantities immediately.
            - Use industry standard rates: Labour ($50-120/hr), Paint ($20-150/L), Timber ($5-20/m).
            ...
            **CRITICAL OUTPUT SCHEMA:**
            Return ONLY a valid JSON object. No markdown.
            {
                "nodes": [
                    { 
                        "id": "ai-string-unique", 
                        "type": "areaNode" | "quoteMaterial" | "quoteLabour" | "materialYield" | "estimationPrism" | "labourEstimator", 
                        "position": { "x": number, "y": number },
                        "data": { 
                            "label": "Item Name (Required)", 
                            "nodeType": "staff" | "equipment" | "material" | "logic",
                            "category": "staff" | "equipment" | "material" | "logic",
                            "quantity": number, 
                            "cost": number,      
                            "rate": number,      
                ...
                "edges": [
                   { "id": "e1", "source": "zone_id", "target": "item_id", "type": "default" }
                ]
            }
            
            **Rules:**
            1. IDs MUST start with 'ai-' (e.g., 'ai-1', 'ai-zone-1').
            2. ALWAYS include at least one **'estimationPrism'** for overall analysis.
            3. Use 'areaNode' for rooms/areas.
            4. Use 'quoteMaterial' for materials and 'quoteLabour' for labor.
            5. Use 'materialYield' for complex coverage calculations (e.g. concrete, paint).
            6. Ensure every node has a descriptive 'label'.
        `;
    ...
    try {
        ...
        const systemPrompt = `
            ...
            **Output Schema:**
            {
                "nodes": [
                    { 
                        "id": "string", 
                        "type": "diaryNode" | "chronos" | "delay" | "impact" | "notesNode", 
            ...
            **Rules:**
            ...
            4. Use 'notesNode' for general observations or text blocks.
            5. Ensure every node has a specific label. NEVER return empty nodes.
        `;
...
const analyzePrismVelocity = async (req, res) => {
    try {
        ...
        const safeContext = {
            ...
            financials: context.projectFinancials || {},
            topology_summary: `${(context.topology || []).length} nodes connected`,
            notes: context.site_notes || []
        };

        const systemPrompt = `
            You are "Neural Progress Prism". ${OMNISCIENCE_MANDATE} ${memory} 
            **Mission:** Analyze live causality, project drift, and financial burn.
            
            **CRITICAL OUTPUT SCHEMA:**
            Return ONLY a valid JSON object matching this exact structure:
            {
                "velocity": number (0.0 to 2.0, where 1.0 is on-track),
                "burnRate": "String (e.g. $450/hr)",
                "predictedFinalMargin": "String (e.g. 18%)",
                "currentMargin": "String (e.g. 22%)",
                "marginRisk": "low" | "medium" | "critical",
                "completionDrift": "String (e.g. +4h)",
                "insights": [
                    { "type": "Efficiency", "severity": "critical" | "warning" | "optimal", "text": "Brief insight", "nodeReferences": [] }
                ],
                "driftStats": {
                    "labour": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "+$0" },
                    "equipment": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "+$0" },
                    "material": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "+$0" },
                    "task": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "0h" },
                    "zone": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "0%" },
                    "cost": { "variancePct": 0, "trend": "up" | "down", "absoluteVariance": "+$0" }
                },
                "scenarios": [
                    { "name": "Optimized", "drift": "-2h", "impact": "If you reduce crew by 1..." }
                ],
                "suggestedNodes": []
            }
            
            **Input Context:** ${JSON.stringify(safeContext)}
        `;
        
        const result = await pinnacleAi.generateJSON("Analyze Prism Velocity", systemPrompt, 3000);
        res.json(result);
    } catch (error) {
...
const generateNodeSuggestions = async (req, res) => {
    try {
        ...
        const systemPrompt = `
            ...
            **INTELLIGENCE:**
            - Check 'ASSOCIATION RULES' in memory. If 'Selected' node matches an 'Item A' rule, you MUST suggest 'Item B' with high priority.
...
const chatSafetyAssistant = async (req, res) => {
    try {
        ...
        try {
            ...
            console.error("Safety Assistant AI Failed:", aiError);
            result = {
                reply: "I am unable to access the latest safety regulations at this moment. Please proceed with standard protocols.",
                suggestedActions: []
            };
        }
        ...
        console.error("Safety Assistant Fatal Error:", e);
        res.json({ 
            reply: "System Offline. Manual risk assessment required.", 
            suggestedActions: [] 
        }); 
    }
};
    ...
    try {
        ...
        const systemPrompt = `
            ...
            **PROTOCOL TAU: VOICE-TO-CANVAS**
            Transform this raw site speech into PaintDiary Canvas Nodes.
            
            **MAPPING RULES:**
            - **Staff / Labor / Time:** Use type "chronos".
            - **Materials / Equipment / Items:** Use type "diaryNode".
            - **Delays / Weather / Issues:** Use type "delay".
            ...
            **OUTPUT SCHEMA:**
            ...
            { 
                "actions": [
                    { 
                        "type": "chronos" | "diaryNode" | "delay", 
                        "label": "Item Name (e.g. 'Dave & Team' or 'Gyprock')", 
                        "quantity": number, 
                        "duration": number (hours),
                        "nodeType": "staff" | "material" | "equipment" | "logic" 
                    }
                ] 
            }
        `;
...
const optimizeFleet = async (req, res) => {
    try {
        ...
        const systemPrompt = `
            ...
            **Goals:**
            1. **SAFETY PROTOCOL:** Check 'FATIGUE ALERT' in Context. DO NOT assign these staff to new shifts. Suggest replacements.
            2. **CAPACITY:** Check 'CRITICAL LOAD'. Do not book staff who are already red-lined.
            3. Resolve any double-bookings (Conflicts).
            4. Minimize cost (avoid overtime).
            5. Ensure high-priority projects are staffed.
...
// --- MAP ASSISTANT (GEOSPATIAL CORE ULTRA - SPEED OPTIMIZED) ---
    ...
    try {
        ...
        const systemPrompt = `
            ...
            **Ground Truth Intelligence (Live API Data):**
            ...
            1. **Logistics Optimization:** Use "nearbyLogistics" to suggest supply points.
            2. **Weather Risk:** Use "groundTruth.weather" to warn about rain/wind impact on site activities.
            3. **Modern Routing:** Use "groundTruth.route" for extremely accurate ETA and traffic paths.
            4. **Fleet Maneuvers:** Suggest "propose_ghost_move" for idling resources.
            
            **Input Intelligence Packet:**
            ${JSON.stringify(context || {}).substring(0, 15000)}
        ...
        const result = await pinnacleAi.generateJSON(message, systemPrompt, 2000);
        res.json(result);
    } catch (error) { 
        console.error("Map Assistant Fatal Error:", error);
        res.json({ reply: "Geospatial Core recalibrating.", suggestedActions: [] }); 
    }
};

// --- PROJECT POST-MORTEM (DEEP FORENSICS) ---
const analyzeProjectPostMortem = async (req, res) => {
    try {
        const { projectId } = req.body;
        const project = await Project.findByPk(projectId, {
            include: [
                { model: Quote, where: { status: 'approved' } },
                { model: Diary },
                { model: Invoice }
            ]
        });
        ...
        project.Quotes.forEach(q => {
            totalQuotedCost += parseFloat(q.totalCost);
            const nodes = q.nodes || [];
            nodes.forEach(n => {
                if (n.data?.category === 'material') {
                    quotedMaterials[n.data.label] = (quotedMaterials[n.data.label] || 0) + (n.data.cost * n.data.quantity);
                } else if (n.data?.category === 'staff') {
                    quotedLabor[n.data.label] = (quotedLabor[n.data.label] || 0) + (n.data.cost * n.data.quantity);
                }
            });
        });
        ...
        project.Diaries.forEach(d => {
            totalActualCost += parseFloat(d.totalCost);
            // Parse canvasData or legacy costs
            // (Simplified assumption: cost data is accessible via diary logic or notes for this high-level pass)
        });

        const variance = totalActualCost - totalQuotedCost;
        const isSuccess = variance <= 0;

        // 2. AI FORENSIC ANALYSIS
        const systemPrompt = `
            You are "Post-Mortem Forensic Auditor". 
            **Mission:** Analyze why Project "${project.name}" was ${isSuccess ? 'SUCCESSFUL' : 'UNSUCCESSFUL'}.
            
            **Financials:**
            - Quoted Cost: $${totalQuotedCost}
            - Actual Cost: $${totalActualCost}
            - Variance: $${variance} (${((variance/totalQuotedCost)*100).toFixed(1)}%)
            
            **Diary Logs (Actual Events):**
            ${project.Diaries.map(d => `[${d.date}] ${d.notes}`).join('\n').substring(0, 5000)}
            
            **CRITICAL OUTPUT:**
            Return a JSON object with specific "Learnings" to save to the Corporate Brain.
            {
                "verdict": "Brief summary of why it succeeded/failed.",
                "rootCauses": ["Reason 1", "Reason 2"],
                "learnedRules": [
                    { "title": "Rule Name", "type": "recommendation", "description": "If doing X, always ensure Y." }
                ]
            }
        `;

        const analysis = await pinnacleAi.generateJSON("Analyze Post-Mortem", systemPrompt, 3000);

        // 3. SAVE TO CORPORATE BRAIN
        if (analysis.learnedRules) {
            for (const rule of analysis.learnedRules) {
