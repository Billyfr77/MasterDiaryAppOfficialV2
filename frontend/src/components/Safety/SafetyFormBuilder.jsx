...
const SafetyFormBuilder = ({ onSave, initialData = [] }) => {
  ...
  const addField = (type) => { const newField = { id: Date.now().toString() + Math.random(), type, label: type === 'header' ? 'New Section' : type === 'paragraph' ? '' : `New ${type}`, value: type === 'paragraph' ? 'Enter text here...' : '', required: false, width: '100%', fontSize: type === 'header' ? '2xl' : 'normal', options: type === 'checkbox' || type === 'select' || type === 'radio' ? ['Option 1', 'Option 2'] : [] }; setFields((prev) => [...prev, newField]); };
  const duplicateField = (field) => { const newField = { ...field, id: Date.now().toString() + Math.random() }; setFields(prev => [...prev, newField]); };
  ...
  const generateWithAI = async () => { if (!aiPrompt) return; setAiLoading(true); try { const res = await api.post('/safety/ai-content', { prompt: aiPrompt, mode: 'full_form' }); if (res.data.result && res.data.result.fields) { const aiFields = res.data.result.fields.map(f => ({ ...f, id: Date.now() + Math.random().toString(), width: '100%' })); setFields(aiFields); setShowAIModal(false); } else { alert("AI Response format invalid."); } } catch (err) { console.error("AI Error:", err); alert("Failed to generate form."); } finally { setAiLoading(false); } };
