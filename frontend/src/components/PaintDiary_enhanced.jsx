...
const DiaryToolbar = ({ onExport, onExportPDF, onExportCSV, theme, onThemeToggle }) => {
...
const PaintDiary = () => {
      ...
      if (e.ctrlKey || e.metaKey) {
        switch (e.key) {
          ...
          case 'z':
            if (e.shiftKey) {
              e.preventDefault()
              // Redo functionality could be added
            } else {
              e.preventDefault()
  ...
  const handleExportPDF = () => {
    const doc = new jsPDF()
    doc.setFontSize(20)
    doc.text('Construction Diary Report', 20, 30)
    doc.setFontSize(12)
    doc.text(`Date: ${selectedDate.toLocaleDateString()}`, 20, 50)
    doc.text(`Total Cost: $${totalCost.toFixed(2)}`, 20, 65)
    doc.text(`Total Revenue: $${totalRevenue.toFixed(2)}`, 20, 80)
    doc.text(`Profit: $${(totalRevenue - totalCost).toFixed(2)}`, 20, 95)
    doc.text(`Productivity Score: ${productivityScore}`, 20, 110)
    ...
    doc.autoTable({
      head: [['Time', 'Notes', 'Items', 'Cost']],
      body: tableData,
      startY: 125
    })

    doc.save(`diary-${selectedDate.toISOString().split('T')[0]}.pdf`)
  }
