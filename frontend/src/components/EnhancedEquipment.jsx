...
const ServiceModal = ({ isOpen, onClose, equipment, onAddRecord }) => {
  ...
  const logs = equipment.serviceLog || []
...
const EnhancedEquipment = () => {
  ...
  const handleAddServiceRecord = (id, record) => {
    // In a real app, POST to /equipment/:id/service-log
    // Here we simulate local update
    const updatedEquipment = equipment.map(e => {
      if (e.id === id) {
        return { ...e, serviceLog: [record, ...(e.serviceLog || [])] }
      }
      return e
    })
    ...
    setSelectedForService(prev => ({ ...prev, serviceLog: [record, ...(prev.serviceLog || [])] }))
    
    // Optimistically assume success or fire generic PUT if supported
    // api.put(`/equipment/${id}`, { serviceLog: ... })
  }
