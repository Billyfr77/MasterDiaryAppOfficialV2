// MASTER CODER ENHANCEMENT: Smart Material Icons Added\nimport React, { useState, useEffect, useCallback, useMemo, useRef } from 'react'
import { ReactFlow, MiniMap, Controls, Background, useNodesState, useEdgesState, addEdge } from '@xyflow/react'
import '@xyflow/react/dist/style.css'
import { useDrag, useDrop, useDragLayer } from 'react-dnd'
import { HTML5Backend } from 'react-dnd-html5-backend'
import { Shovel, User, Wrench, Package, Zap, Plus, Save, Sparkles, Search, Volume2, VolumeX, Mountain, BrickWall, TreePine, Hammer, HardHat } from 'lucide-react'
...
const ItemCard = ({ item }) => {
  ...
  const getDisplayInfo = () => {
    ...
    return { label: item.name, sub: `$${item.pricePerUnit}/${item.unit}`, icon: getMaterialIcon(item) }
  }
...
const QuoteBuilder = () => {
  ...
  const [edges, setEdges, onEdgesState, onEdgesChange] = useEdgesState(initialEdges)
